      ...
      <div className="flex-1 min-w-0 relative overflow-hidden">
        {screenshot.panorama ? (
          ...
          <div ref={wrapRef} className="w-full h-full flex items-center justify-center relative">
            ...
            {/* Zoom badge */}
            {(animating || isZoomed) && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 px-3 py-1 rounded-md bg-black/70 text-xs text-[#ccc] pointer-events-none z-30">
                {Math.round(zoom * 100)}%
              </div>
            )}
            {/* Minimap */}
            {isZoomed && viewport.w > 0 && screenshot.width && screenshot.height && (
              <div className="absolute bottom-6 right-6 w-28 h-[4.5rem] rounded-md overflow-hidden border border-white/20 bg-black/70 shadow-lg pointer-events-none z-30">
